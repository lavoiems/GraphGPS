...
class SEM(torch.nn.Module):
    def __init__(self, in_dim, L, V, tau):
        super(SEM, self).__init__()
        self.embedder = torch.nn.Sequential(
                        torch.nn.Linear(in_dim, L*V, bias=False),
                        torch.nn.BatchNorm1d(L*V),
                        )
        self.L = L
        self.V = V
        self.tau = tau
...
@register_network('GPSSEMModel')
class GPSSEMModel(torch.nn.Module):
    ...
    def __init__(self, dim_in, dim_out):
        ...
        self.post_mp_sem = SEM(cfg.gnn.dim_inner, L, V, tau)

        GNNHead = register.head_dict[cfg.gnn.head]
        self.post_mp = GNNHead(dim_in=L*V, dim_out=dim_out)
