...
def distill_epoch(logger, loader, student, teacher, optimizer, scheduler, batch_accumulation, L, V):
    student.train()
    teacher.eval()
    total_loss = 0
    #time_start = time.time()
    for batch in loader:
        ...
        loss = torch.nn.functional.cross_entropy(o_student.reshape(-1,V)/3, teach_label.reshape(-1,))
...
def custom_train(loggers, loaders, model, optimizer, scheduler):
    ...
    for cur_iteration in range(start_iteration, cfg.optim.num_iteration):
        ...
        distill_cfg.optim.weight_decay = 0.01
        ...
        for cur_epoch in range(start_epoch, cfg.optim.distill_epoch):
            ...
            loss = distill_epoch(logging, loaders[0], new_model, model, optimizer, scheduler, cfg.optim.batch_accumulation, cfg.sem.L, cfg.sem.V)
            logging.info(f'Distill loss: {loss}')
            if is_eval_epoch(total_epoch):
                for i in range(1, num_splits):
